...
> - 译文地址：[理解异步 JavaScript -了解 JavaScript 工作原理](https://github.com/h246802/blogs/issues/2)
...
在我们深入研究异步 JavaScript 之前，让我们首先了解同步 JavaScript 代码在 JavaScript 引擎中的执行情况。例如：
...
JavaScript 有一个单独的调用堆栈，因为它是一种单线程编程语言。调用堆栈具有 LIFO 结构，这意味着只能从堆栈顶部添加或删除项目。
...
![image](https://cdn-images-1.medium.com/max/1240/1*DkG1a8f7rdl0GxM0ly4P7w.png)
 ...
 接下来，console.log('Hi there!') 被推到堆栈的顶部，当它执行完成后，它会从堆栈中弹出。在它之后，我们调用 second() ，因此 second() 函数被推送到堆栈的顶部。

 console.log('Hello there!') 被推到堆栈顶部并在完成后从堆栈中弹出。second() 函数执行完成，接着它从堆栈中弹出。

 console.log('The End') 被推到堆栈顶部并在完成后被删除。之后，first() 函数执行完成，因此它从堆栈中删除。

 程序此时完成其执行，因此从堆栈中弹出全局执行上下文 (main())。
...
当 `processImage()` 函数完成时，它将从堆栈中删除。之后调用`networkRequest()` 函数并将其推送到执行栈。同样，它还需要一些时间才能完成执行。

最后，当 `networkRequest()` 函数完成时，调用 `greeting()` 函数，因为它只包含 `console.log` 语句，而console.log语句通常很快，所以 `greeting()` 函数会立即执行并返回。

所以你可以看到，我们必须等到函数（例如 `processImage()` 或 `networkRequest()` ）完成。这也就意味着这些函数阻塞了调用堆栈或主线程。因此，在执行上述代码时，我们无法执行任何其他操作，这是不理想的。
...
![image](https://cdn-images-1.medium.com/max/992/1*sOz5cj-_Jjv23njWg_-uGA.gif))
 ...
 `setTimeout()` 方法在 Web APIs 环境中启动 2s 的计时器。此时，`setTimeout()` 已完成，并从堆栈中弹出。在它之后，`console.log('The End')` 被推送到栈，在执行完成后从堆栈中删除。
...
在这个例子中，消息队列包含一个回调，此时调用堆栈为空。因此，事件循环（Event Loop） 将回调推送到堆栈顶部。

再之后，`console.log('Async Code')` 被推到栈顶部，执行并从堆栈中弹出。此时，回调函数已完成，因此将其从堆栈中删除，程序最终完成。
...
事件循环再次检查调用堆栈是否为空，如果它为空并且执行了回调，则将事件回调推送到堆栈。
...
**译者注：**
- 文中 工作队列（Job Queue）也就是微任务队列，而宏任务队列则被消息队列代指